...
import { COINGECKO_URL, ZORA_ADDRESS }  from "../../../utils/const";
...
const CreatePage = () => {
    ...
    const loadData = async () => {
        if (wallet && wallet.ethereum) {
            const provider = new ethers.providers.Web3Provider(wallet.ethereum);
            const network = await provider.getNetwork();
            setRightToken(ZORA_ADDRESS[network.chainId]);
            try {
            ...
            loadTokens();
            getTokenBalance(ZORA_ADDRESS[network.chainId]);
        }
    }
    ...
    const onCreateSwap = async () => {
        ...
        console.log("rightTokenAmount", rightTokenAmount, rightToken.decimals)
        let tAmount = "0";
        if (type_right === "r_erc20") {
            tAmount = (new BigNumber(10).exponentiatedBy(rightToken.decimals).multipliedBy(rightTokenAmount)).toString();
        }
        try {
            await createSwap(
        ...
        <Flex maxW="70rem" flexDirection="column" w="100%" m="3rem auto" p="0 1rem">
            ...
            >
                ...
                {mycontext.nftToken && leftTokenApproved?
                    ...
                    <Flex bg="#ccc" borderRadius="30px" color="white" p="0 1rem" ml="1rem" cursor="pointer" userSelect="none">
                        <Text m="auto 0" fontSize="12px" fontWeight="bold">Create Swap</Text>
                    </Flex>
                }
                </Flex>
