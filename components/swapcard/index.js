...
const SwapCard = (props) => {
    ...
    const openTokenLink = async (addr, id) => {
        if (wallet && wallet.ethereum) {
            const provider = new ethers.providers.Web3Provider(wallet.ethereum);
            const network = await provider.getNetwork();
            if (network.chainId === 1) window.open("https://opensea.io/assets/" + addr + "/" + id);
            else if (network.chainId === 4) window.open("https://testnets.opensea.io/assets/" + addr + "/" + id);
        }
    }
    
    const openErc20TokenLink = async (addr) => {
        if (wallet && wallet.ethereum) {
            const provider = new ethers.providers.Web3Provider(wallet.ethereum);
            const network = await provider.getNetwork();
            if (network.chainId === 1) window.open("https://etherscan.io/token/" + addr);
            else if (network.chainId === 4) window.open("https://rinkeby.etherscan.io/token/" + addr);
        }
    }

    const goSwapPage = () => {
        router.push("/nftswap/" + swap.id);
    }
            ...
            <Flex flexDirection="column">
              <Box w="12rem" p="0.5rem" border="1px solid #ccc" borderRadius="10px">
                ...
                <Box mt="1rem" mb="0.2rem">
                    <Flex>
                        <Box>
                            <Text fontWeight="bold" fontSize="12px" color="#000">
                                {nftToken.name}
                            </Text>
                        </Box>
                    </Flex>
        ...
        return (
            <Flex flexDirection="column">
                <Flex w="12rem" h="100%" p="0.5rem" border="1px solid #ccc" borderRadius="10px" flexDirection="column" justifyContent="center">
                    ...
                    <Text fontSize="18px" fontWeight="bold" textAlign="center" color="#000">{token.symbol}</Text>
                    <Text textAlign="center" color="#000">{parseInt(token.amount) / Math.pow(10, token.decimals)}</Text>
                </Flex>
                <Box w="12rem">
                    <Box bg="white" color="blue.900" m="1rem" p="0.5rem" borderRadius="30px"
                        border="1px solid #6095FF"
    ...
    return (
        ...
        <Box w="100%" p="0.5rem" borderRadius="10px" border="1px solid #ccc">
            <Text fontSize="12px" fontWeight="bold" mb="0.5rem" color="#000">Left Amount: {swap.leftAmount}</Text>
